...
class DatasetLLFF(Dataset):
    def __init__(self, base_dir, FLAGS, examples=None):
        ...
        )  # Taken from nerf, swizzles from LLFF to expected coordinate system
        poses = np.moveaxis(poses, -1, 0).astype(np.float32)

        lcol = np.array([0, 0, 0, 1], dtype=np.float32)[None, None, :].repeat(
            poses.shape[0], 0
        )
        self.imvs = torch.tensor(
            np.concatenate((poses[:, :, 0:4], lcol), axis=1), dtype=torch.float32
        )
        ...
        if self.FLAGS.local_rank == 0:
            print(
                "DatasetLLFF: %d images with shape [%d, %d]"
                % (len(all_img), self.resolution[0], self.resolution[1])
            )
    ...
    def _parse_frame(self, idx):
        ...
        all_mask = [
            f
            for f in sorted(glob.glob(os.path.join(self.base_dir, "masks", "*")))
            if f.lower().endswith("png")
            or f.lower().endswith("jpg")
            or f.lower().endswith("jpeg")
        ]
        assert (
            len(all_img) == self.imvs.shape[0] and len(all_mask) == self.imvs.shape[0]
        )
