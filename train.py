...
class Trainer(torch.nn.Module):
    ...
    def forward(self, target, it):
        ...
        return self.geometry.tick(
            glctx, target, self.light, self.material, self.image_loss_fn, it
        )
...
if __name__ == "__main__":
    ...
    FLAGS.laplace_scale = (
        10000.0  # Weight for sdf regularizer. Default is relative with large weight
    )
    ...
    if os.path.splitext(FLAGS.ref_mesh)[1] == ".obj":
        ...
        if os.path.isfile(os.path.join(FLAGS.ref_mesh, "poses_bounds.npy")):
            dataset_train = DatasetLLFF(
                FLAGS.ref_mesh, FLAGS, examples=(FLAGS.iter + 1) * FLAGS.batch
            )
            ...
            dataset_train = DatasetNERF(
                os.path.join(FLAGS.ref_mesh, "transforms_train.json"),
                FLAGS,
