...
class _masking_smith_func(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        return _get_plugin().masking_smith_bwd(alphaSqr, cosThetaI, cosThetaO, dout) + (
            None,
        )
...
def specular_cubemap(cubemap, roughness, cutoff=0.99, use_python=False):
    assert (
        cubemap.shape[0] == 6 and cubemap.shape[1] == cubemap.shape[2]
    ), "Bad shape for cubemap tensor: %s" % str(cubemap.shape)
...
class _xfm_func(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        return (_get_plugin().xfm_bwd(points, matrix, dout, ctx.isPoints),) + (
            None,
            None,
            None,
        )
