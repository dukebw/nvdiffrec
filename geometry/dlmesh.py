...
class DLMesh(torch.nn.Module):
    def __init__(self, initial_guess, FLAGS):
        ...
        print(
            "Base mesh has %d triangles and %d vertices."
            % (self.mesh.t_pos_idx.shape[0], self.mesh.v_pos.shape[0])
        )
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration):
        ...
        if self.FLAGS.laplace == "absolute":
            reg_loss += (
                regularizer.laplace_regularizer_const(
                    self.mesh.v_pos, self.mesh.t_pos_idx
                )
                * self.FLAGS.laplace_scale
                * (1 - t_iter)
            )
        elif self.FLAGS.laplace == "relative":
            reg_loss += (
                regularizer.laplace_regularizer_const(
                    self.mesh.v_pos - self.initial_guess.v_pos, self.mesh.t_pos_idx
                )
                * self.FLAGS.laplace_scale
                * (1 - t_iter)
            )
        ...
        reg_loss += (
            torch.mean(buffers["kd_grad"][..., :-1] * buffers["kd_grad"][..., -1:])
            * 0.03
            * min(1.0, iteration / 500)
        )

        # Visibility regularizer
        reg_loss += (
            torch.mean(buffers["occlusion"][..., :-1] * buffers["occlusion"][..., -1:])
            * 0.001
            * min(1.0, iteration / 500)
        )
