...
class DMTetGeometry(torch.nn.Module):
    def __init__(self, grid_res, scale, FLAGS):
        ...
        self.verts = (
            torch.tensor(tets["vertices"], dtype=torch.float32, device="cuda") * scale
        )
    ...
    def getMesh(self, material):
        ...
        verts, faces, uvs, uv_idx = self.marching_tets(
            v_deformed, self.sdf, self.indices
        )
    ...
    def tick(self, glctx, target, lgt, opt_material, loss_fn, iteration):
        ...
        sdf_weight = self.FLAGS.sdf_regularizer - (
            self.FLAGS.sdf_regularizer - 0.01
        ) * min(1.0, 4.0 * t_iter)
        reg_loss = (
            sdf_reg_loss(self.sdf, self.all_edges).mean() * sdf_weight
        )  # Dropoff to 0.01
