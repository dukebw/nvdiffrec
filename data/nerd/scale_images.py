...
for dataset in datasets:
    ...
    shutil.copyfile(
        os.path.join(dataset, "poses_bounds.npy"),
        os.path.join(dataset_rescaled, "poses_bounds.npy"),
    )
    for folder in folders:
        os.makedirs(os.path.join(dataset_rescaled, folder), exist_ok=True)
        files = glob.glob(os.path.join(dataset, folder, "*.jpg")) + glob.glob(
            os.path.join(dataset, folder, "*.JPG")
        )
        for file in files:
            print(file)
            ...
            imageio.imwrite(
                out_file,
                np.clip(np.rint(rescaled_img.numpy() * 255.0), 0, 255).astype(np.uint8),
            )
