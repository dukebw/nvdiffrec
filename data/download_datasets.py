...
def download_nerd():
    ...
    for dataset in datasets:
        ...
        # Download dataset
        wget = requests.get(
            "https://github.com/vork/%s/archive/refs/heads/master.zip" % dataset,
            stream=True,
        )
        ...
        for zipinfo in archive.infolist():
            out_file = zipinfo.filename.replace(
                "%s-master/" % dataset, "%s/" % dataset_rescaled
            )
            ext = os.path.splitext(out_file)[1][1:]
            if ext.lower() == "jpg":
                folder = os.path.dirname(out_file)
                img = torch.tensor(
                    imageio.imread(archive.read(zipinfo.filename), format=ext).astype(
                        np.float32
                    )
                    / 255.0
                )
                ...
                imageio.imwrite(
                    os.path.join("nerd", out_file),
                    np.clip(np.rint(rescaled_img.numpy() * 255.0), 0, 255).astype(
                        np.uint8
                    ),
                )
            else:
